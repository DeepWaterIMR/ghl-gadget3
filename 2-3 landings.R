...
if(reload_data) {
  ...
  RussianSurvey_landings <- structure(
    data.frame(
      year = unique(Russian_SI$year), step = 1, area = 1, total_weight = 1),
    area_group = mfdb_group(`1` = 1))

  png(file.path(base_dir, "figures/RussianSurvey_landings.png"), width = pagewidth, height = pagewidth*0.7, units = "mm", res = 300)
  print(plot.landings(RussianSurvey_landings))
  dev.off()

  # Save

  save(HistNor_landings, TrawlNor_landings, OtherNor_landings, TrawlRus_landings, OtherRus_landings, Internat_landings, EggaN_landings, RussianSurvey_landings, file = file.path(base_dir, "data/Landings to Gadget.rda")) #  HistRus_landings,
