...
proj_stock_actions2 <- function(num_project_years,
  ...
  out_stocks <- list(imm_F, imm_M)
  
  ## Setup up a dummy stock
  dummy_stock <- 
    gadget3::g3_stock(c(species = mat$name_parts[['species']], 'dummy'),
                      lengthgroups = seq(min(gadget3::g3_stock_def(imm_F, 'minlen')),
                                         max(head(gadget3::g3_stock_def(imm_F, 'maxlen'),-1)),
  ...
  spawning_actions <- 
    list(
      gadget3::g3a_spawn(
        ...
        run_f = gadget3:::f_substitute(~cur_year > (end_year - (minage + 1)) && cur_step == 1,
                                       list(end_year = local(end_year),
                                            minage = gadget3::g3_stock_def(imm_F, 'minage'))))
...
runfun <- function(adfun, pars, hr_len = 45){
