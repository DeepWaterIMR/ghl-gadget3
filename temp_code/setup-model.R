...
## stocks
female_imm <-
  g3_stock(c(species = tolower(defaults$species), sex = 'female', 'imm'), 
           lengthgroups = seq(4, 100,2)) %>%
  g3s_livesonareas(areas[c('1')]) %>%
  g3s_age(minage = 1, maxage = 10)

male_imm <-
  g3_stock(c(species = tolower(defaults$species), sex = 'male', 'imm'),
           lengthgroups = seq(4, 100, 2)) %>%
  g3s_livesonareas(areas[c('1')]) %>%
  g3s_age(minage = 1, maxage = 10)

female_mat <-
  g3_stock(c(species = tolower(defaults$species), sex = 'female', 'mat'),
           lengthgroups = seq(20, 120, 2)) %>%
  g3s_livesonareas(areas[c('1')]) %>%
  g3s_age(minage = 3, maxage = 25)

male_mat <- 
  g3_stock(c(species = tolower(defaults$species), sex = 'male', 'mat'), 
           lengthgroups = seq(20, 120, 2)) %>%
  g3s_livesonareas(areas[c('1')]) %>%
  g3s_age(minage = 3, maxage = 25)
...
# g3_param_table(init, expand.grid(age = seq(1,25))), lower, upper),
# c(model_params,
#   model_params$init,
...
#     model_params$renew,
#     scalar = bounded_param(male_imm,id=c('species','sex'), "scalar", model_params),
#     renew = paste(g3_stock_name(male_imm,id=c('species','sex')), "init", sep=".")))
# 
# 
...
#     model_params$renew,
#     scalar = bounded_param(female_imm,id=c('species','sex'), "scalar", model_params),
#     renew = paste(g3_stock_name(female_imm,id=c('species','sex')), "init", sep=".")))

## Ensure that old fish are not immature
# a50 is bounded
prop_m_age <- ~ 1/(1 + exp(-mat.a*(age - a50)))
f_prop_m_age <- 
  gadget3:::f_substitute(prop_m_age, 
...
female_imm_actions <- list(
  ...
  g3a_initialconditions_normalparam(female_imm,
                                    ...
                                    alpha_f = bounded_param(female_imm, "walpha", model_params),
                                    beta_f = bounded_param(female_imm, "wbeta", model_params)),
  
  g3a_renewal_normalparam(female_imm,
                          ...
                          alpha_f = bounded_param(female_imm,id=c('species'), "walpha", model_params),
                          beta_f = bounded_param(female_imm,id=c('species'), "wbeta", model_params),
                          run_f = ~cur_step == 1 && age == 1 && cur_time > 0),
  
  g3a_growmature(female_imm,
                 impl_f = g3a_grow_impl_bbinom(
                   g3a_grow_lengthvbsimple(bounded_param(female_imm,id=c('species','sex'), "Linf", model_params),
                                           gadget3:::f_substitute(~0.001 * K, 
                                                                  list(K = bounded_param(female_imm,id=c('species','sex'), "K", model_params)))),      
                   g3a_grow_weightsimple(bounded_param(female_imm,id=c('species','sex'), "walpha", model_params),  
                                         bounded_param(female_imm,id=c('species','sex'), "wbeta", model_params)),   
...
male_imm_actions <- list(
  ...
  g3a_initialconditions_normalparam(male_imm,
                                    # NB: area & age factor together (gadget2 just multiplied them)
                                    # initial abundance at age is 1e4 * q
                                    factor_f =
                                      gadget3:::f_substitute(
                                    ...
                                    alpha_f = bounded_param(male_imm, "walpha", model_params),
                                    beta_f = bounded_param(male_imm, "wbeta", model_params)),
  ...
  g3a_renewal_normalparam(male_imm,
                          ...
                          alpha_f = bounded_param(male_imm,id=c('species'), "walpha", model_params),
                          beta_f = bounded_param(male_imm,id=c('species'), "wbeta", model_params),
                          run_f = ~cur_step == 1 && age == 1 && cur_time > 0),
  
  g3a_growmature(male_imm,
                 impl_f = g3a_grow_impl_bbinom(
                   g3a_grow_lengthvbsimple(bounded_param(male_imm,id=c('species','sex'), "Linf", model_params),
                                           gadget3:::f_substitute(~0.001 * K, 
                                                                  list(K = bounded_param(male_imm,id=c('species','sex'), "K", model_params)))),      
                   g3a_grow_weightsimple(bounded_param(male_imm,id=c('species','sex'), "walpha", model_params),  
                                         bounded_param(male_imm,id=c('species','sex'), "wbeta", model_params)),   
...
female_mat_actions <- list(
  g3a_initialconditions_normalparam(female_mat,
                                    # NB: area & age factor together (gadget2 just multiplied them)
                                    # initial abundance at age is 1e4 * q
                                    factor_f =
                                      gadget3:::f_substitute(
                                    ...
                                    alpha_f = bounded_param(female_mat, "walpha", model_params),
                                    beta_f = bounded_param(female_mat, "wbeta", model_params)),
  ...
  g3a_growmature(female_mat,
                 impl_f = g3a_grow_impl_bbinom(
                   ...
                   g3a_grow_weightsimple(bounded_param(female_mat,id=c('species','sex'), "walpha", model_params),  
                                         bounded_param(female_mat,id=c('species','sex'), "wbeta", model_params)),   
...
male_mat_actions <- list(
  g3a_initialconditions_normalparam(male_mat,
                                    # NB: area & age factor together (gadget2 just multiplied them)
                                    # initial abundance at age is 1e4 * q
                                    factor_f =
                                      gadget3:::f_substitute(
                                    ...
                                    alpha_f = bounded_param(male_mat, "walpha", model_params),
                                    beta_f = bounded_param(male_mat, "wbeta", model_params)),
  
  g3a_growmature(male_mat,
                 impl_f = g3a_grow_impl_bbinom(
                   g3a_grow_lengthvbsimple(bounded_param(male_mat,id=c('species','sex'), "Linf", model_params),
                                           gadget3:::f_substitute(~0.001 * K, 
                                                                  list(K = bounded_param(male_mat,id=c('species','sex'), "K", model_params)))),      
                   g3a_grow_weightsimple(bounded_param(male_mat,id=c('species','sex'), "walpha", model_params),  
                                         bounded_param(male_mat,id=c('species','sex'), "wbeta", model_params)),   
