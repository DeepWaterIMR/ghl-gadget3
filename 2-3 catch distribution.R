...
if(reload_data) {
  ...
  nor_catch_aldist <- readRDS("../ghl-gadget-data/data/out/Age length data for catches.rds")
  
  # Linf (move somewhere)
  
  # nor_catch_ldist %>% # or nor_survey_ldist
  #   filter(!is.na(sex), !is.na(length)) %>% 
  #   # filter(sampling_type %in% c("ENS", "ESS")) %>% 
  #   # filter(gear %in% "BottomTrawls") %>% 
  #   group_by(sex) %>% 
  #   summarise(quantile = c(0.01, 0.05, 0.25, 0.5, 0.75, 0.95, 0.99),
  #             length = quantile(length, quantile))
  ...
  OtherNor_aldist <- g3_data(
    ...
    method = "est_n",
    verbose = FALSE
  ) %>% rename("number" = "est_n") %>% filter(!year %in% c(2019))
